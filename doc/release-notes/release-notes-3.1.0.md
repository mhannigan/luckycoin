LUK Core version *3.1.0* is now available from:  <https://github.com/luckycoin-project/luckycoin/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/luckycoin-project/luckycoin/issues>


Mandatory Update
==============

LUK Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zLUK Staking is here! zLUK staking will be activated on the morning of the 8th of May 2018. With the release of zLUK staking, there are effectively 2 versions of zLUK, zLUK minted on the 3.0.6 LUK wallet or lower, and zLUK minted on LUK wallet version or higher. New features in this release will require the use of zLUK v2, zLUK minted on this wallet release 3.1.0 or later. If you currently hold zLUK v1 and wish to take advantage of zLUK staking and deterministic zLUK, you will need to spend the zLUK v1 to yourself and remint zLUK v2.
Note: To find your zLUK version, click the privacy tab, then the zLUK Control button then expand the arrows next to the desired denomination.


### Deterministic zLUK Seed Keys

zLUK is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zLUK outside of the wallet that the zLUK had been minted on. zLUK can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zLUK minting

zLUK minting now only requires 1 further mint (down from 2) to mature. zLUK mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zLUK Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zLUK denomination has been spent or not.



LUK/zLUK Staking and Masternode Rewards
--------------

### LUK, zLUK and Masternode Payment Schedule

To encourage the use of zLUK and increase the LUK zerocoin anonymity set, the LUK payment schedule has been changed to the following:

If a user staking zLUK wins the reward for their block, the following zLUK reward will be: 
- 3 zLUK (3 x 1 denominations) rewarded to the staker, 2 LUK rewarded to the masternode owner and 1 LUK available for the budget. This is a total block reward of 6 LUK, up from 5.

If a user staking LUK wins the reward, the following amounts will be rewarded: 
- 2 LUK to the LUK staker, 3 LUK to the Masternode owner and 1 LUK available for the budget. This is a total block reward of 6 LUK, up from 5.


### Return change to sender when minting zLUK

Previously, zLUK minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most LUK and return the change from a zLUK mint to that address.
...
The visual layout of the LUK Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zLUK minting feature. This is enabled by default and the enablezeromint=0 setting in the luckycoin.conf file will overwrite the GUI option.
-	The percentage of autominted zLUK can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular LUK.
...
### Summary
 ...
 The LUK wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the luckycoin.conf file, found in the LUK data directory.
...
* If zLUK backup, auto generated name is `wallet-autozpivbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
LUK Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The LUK client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running luckycoind without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the LUK support group, which often times sees users say "I didn't spend that zLUK". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the LUK masternodes, the monthly budget available to be utilised has been increased to 42,000 LUK / month. This LUK only has the opportunity to be raised once per month (paid to winning proposals) with any unused LUK not created by the blockchain.

### Budget Finalization Fee

The LUK finalization fee for successful proposals has now been reduced, this fee is now 5 LUK down from 50 LUK. The total fee outlay for a successful proposal is now a total of 55 LUK.
...
-	Enforced v1 zLUK spends to require a security level of 100
-	Updates to zLUK spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the LUK-Qt masternodes tab
-	Updated LUK-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and LUK's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zLUK Control dialog in the QT wallet, a user is able to see zLUK marked as version 1 or 2.

zPoS (zLUK staking)
--------------

Once a zLUK has over 200 confirmations it becomes available to stake. Staking zLUK will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zLUK. So for example if a 1,000 zLUK denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zLUKs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within LUK's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zLUK, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zLUK that can be recalculated at any time using the seed. Deterministic zLUK allows for users to backup all of their future zLUK by simply recording their seed and keeping it in a safe place (similar to backing up a private key for LUK). The zLUK seed needs to remain in the wallet in order to spend the zLUK after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zLUK data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

LUK's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which LUK used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to LUK using the bug bounty program. The modulus is now correctly imported and LUK's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zLUK v2: zPoS dzLUK ezLUK external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zLUK balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zLUK status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault luckycoin-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zLUK spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zLUK. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zLUK. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/luckycoin-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
