...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LUK at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLUK send
enum ZerocoinSpendStatus {
    ZLUK_SPEND_OKAY = 0,                            // No error
    ZLUK_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLUK_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLUK_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLUK_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLUK_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLUK_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLUK_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLUK_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLUK_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLUK_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLUK_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLUK_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLUK_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLUK_SPENT_USED_ZLUK = 14,                      // Coin has already been spend
    ZLUK_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLUK_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZLUKOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzLUKWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzLUKTracker> zpivTracker;
    ...
    void setZWallet(CzLUKWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzLUKTracker>(new CzLUKTracker(strWalletFile));
    }

    CzLUKWallet* getZWallet() { return zwalletMain; }
    ...
    /** zLUK reset */
    boost::signals2::signal<void()> NotifyzLUKReset;
