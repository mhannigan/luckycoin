...
public:
    ...
    bool WriteZLUKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLUKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLUKSeed_deprecated(uint256& seed);
    bool EraseZLUKSeed();
    bool EraseZLUKSeed_deprecated();

    bool WriteZLUKCount(const uint32_t& nCount);
    bool ReadZLUKCount(uint32_t& nCount);
