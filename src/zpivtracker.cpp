...
CzLUKTracker::CzLUKTracker(std::string strWalletFile)
{
...
CzLUKTracker::~CzLUKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLUKTracker::Init()
{
...
bool CzLUKTracker::Archive(CMintMeta& meta)
{
...
bool CzLUKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLUKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLUKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLUKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLUKTracker::GetSerialHashes()
{
...
CAmount CzLUKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLUKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLUKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLUKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLUKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLUKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLUKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLUKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLUKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLUKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLUKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLUKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLUKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLUKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLUKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLUKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLUKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
