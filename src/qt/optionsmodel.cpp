...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LUK);
    ...
    if (!settings.contains("nAnonymizeLuckyCoinAmount"))
        settings.setValue("nAnonymizeLuckyCoinAmount", 1000);

    nAnonymizeLuckyCoinAmount = settings.value("nAnonymizeLuckyCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLuckyCoinAmount"))
        SoftSetArg("-anonymizeluckycoinamount", settings.value("nAnonymizeLuckyCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in luckycoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLuckyCoinAmount:
            return QVariant(nAnonymizeLuckyCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLuckyCoinAmount:
            nAnonymizeLuckyCoinAmount = value.toInt();
            settings.setValue("nAnonymizeLuckyCoinAmount", nAnonymizeLuckyCoinAmount);
            emit anonymizeLuckyCoinAmountChanged(nAnonymizeLuckyCoinAmount);
            break;
        case CoinControlFeatures:
