...
CzLUKWallet::CzLUKWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLUKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLUKSeed_deprecated()) {
                    LogPrintf("%s: Updated zLUK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzLUKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLUKCount(nCountLastUsed);
    else if (!walletdb.ReadZLUKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLUKWallet::Lock()
{
    seedMaster = 0;
}

void CzLUKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLUKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLUK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLUKWallet::LoadMintPoolFromDB()
{
...
void CzLUKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLUKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLUKWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzLUKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLUK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzLUKWallet::SeedToZLUK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLUKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLUKWallet::GenerateDeterministicZLUK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLUKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLUKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
